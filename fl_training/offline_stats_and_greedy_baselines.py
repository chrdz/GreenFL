...
for root in [_THIS.parents[0], _THIS.parents[1], _THIS.parents[2], _THIS.parents[3], Path.cwd()]:
    if (root / "fl_training").exists() and (root / "building_availability_matrices").exists():
        _REPO_ROOT = root
        break
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--T", type=int, required=True, help="Base training horizon (rounds).")
    ...
    p.add_argument("--window-out-folder", type=str, default=str(_REPO_ROOT / "building_availability_matrices"), help="Path that contains historical_data/ (passed to Window(..., out_folder=...)).")
    p.add_argument("--start-time", type=str, default=None, help="ISO start time. If omitted, uses random_start.")
    p.add_argument("--random-start", action=argparse.BooleanOptionalAction, default=True)

    # ---- Offline stats options
    p.add_argument("--experiment", type=str, required=True, help="mnist, cifar10, cifar100, femnist, etc.")
    ...
    p.add_argument("--offline-stats-path", type=str, default="", help="If set and exists, load stats from here instead of recomputing.")
    p.add_argument("--save-offline-stats", type=str, default="", help="If set, save computed stats to this path (.pt).")

    # ---- Solver options
    p.add_argument("--s-step", type=int, default=1, help="Evaluate s in {1, 1+s_step, ...} up to t_sl.")

    # ---- Output & plotting
    p.add_argument("--out-dir", type=str, default="availability_matrices")
    p.add_argument("--name-prefix", type=str, default="")
    p.add_argument("--plot", action="store_true")
    p.add_argument("--plot-save", type=str, default="", help="If set, saves comparison plot to this path (png/pdf).")
...
def build_probe_clients(experiment: str, batch_size: int, verbose: int) -> List[OfflineProbeClient]:
...
def compute_or_load_offline_stats(args: argparse.Namespace) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if args.verbose:
        print("==> Computing CAFE (distance matrix) and FedZero (sigma) offline stats..")
    ...
    if args.verbose:
        ...
        print(f"  Sigma stats: min={sigma.min():.4f}, max={sigma.max():.4f}, mean={sigma.mean():.4f}")

    return D, sigma


def load_ghg_matrix(args: argparse.Namespace, countries: List[str]) -> Tuple[np.ndarray, List[str]]:
    ...
    if start_time is None and not args.random_start:
        if args.verbose:
            print("[GreenFL] WARNING: --no-random-start with no --start-time can crash Window. Enabling random_start=True.")
...
def solve_problem1_alpha_fair_greedy(
    GHG_mat: np.ndarray, T: int, t_sl: int, t_ft: int, alpha: float, budget_k: float, s_step: int = 1
) -> SolveResult:
    K, H = GHG_mat.shape
    ...
    if best_A is None or best_s is None:
        raise RuntimeError("Problem 1: no feasible solution found (forced FT likely exceeds budget).")
    return SolveResult(A_full=best_A, s_best=best_s, objective_value=best_val)


def solve_problem2_cafe_greedy(
    GHG_mat: np.ndarray, D: np.ndarray, T: int, t_sl: int, t_ft: int, budget_k: float, s_step: int = 1
) -> SolveResult:
    K, H = GHG_mat.shape
    if D.shape != (K, K):
    ...
    if best_A is None or best_s is None:
        raise RuntimeError("Problem 2: no feasible solution found (forced FT likely exceeds budget).")
    return SolveResult(A_full=best_A, s_best=best_s, objective_value=best_val)


def solve_problem3_fedzero_greedy(
    GHG_mat: np.ndarray, sigma: np.ndarray, T: int, t_sl: int, t_ft: int, budget_k: float, s_step: int = 1
) -> SolveResult:
    K, H = GHG_mat.shape
    ...
    if best_A is None or best_s is None:
        raise RuntimeError("Problem 3: no feasible solution found (forced FT likely exceeds budget).")
    return SolveResult(A_full=best_A, s_best=best_s, objective_value=best_val)


def save_availability(A_full: np.ndarray, row_labels: List[str], out_dir: Path, name: str, T: int, s_best: int) -> Tuple[Path, Path]:
    import pandas as pd
...
def plot_three_matrices(
    ...
    for ax, (label, res) in zip(axs, results):
        ...
        ax.set_title(f"{label} | {t_sl}sl-{budget}cb-{t_ft}ft | best s={res.s_best}, obj={res.objective_value:.6f}")
